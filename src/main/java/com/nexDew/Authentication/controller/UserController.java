...
public class UserController {
    ...
    public ResponseEntity<APIResponse<List<AuthEntityDTO>>> getAllUsers() {
    ...
    @GetMapping("/{uuid}")
    @PreAuthorize("hasAuthority('USER_READ') or #uuid == authentication.principal.id")
    public ResponseEntity<APIResponse<AuthEntityDTO>> getUserById(@PathVariable UUID uuid) {
        return ResponseEntity.ok(
                APIResponse.success("User fetched successfully", userService.getUserById(uuid))
        );
    }

    @PostMapping
    @PreAuthorize("hasAuthority('USER_WRITE')")
    public ResponseEntity<APIResponse<AuthEntityDTO>> createUser(@RequestBody CreateUserRequest request) {
    ...
    public ResponseEntity<APIResponse<AuthEntityDTO>> updateUser(
    ...
    @PreAuthorize("hasAuthority('USER_WRITE')")
    public ResponseEntity<APIResponse<AuthEntityDTO>> assignRole(
            @PathVariable UUID id,
    ...
    @PreAuthorize("hasAuthority('USER_WRITE')")
    public ResponseEntity<APIResponse<AuthEntityDTO>> removeRole(
            @PathVariable UUID id,
            @RequestBody Set<Long> roleIds) {
    ...
    public ResponseEntity<APIResponse<Set<PermissionDTO>>> getPermissions(@PathVariable UUID id) {
        return ResponseEntity.ok(
    ...
    @GetMapping("/{id}/permissions/{permissionName}")
    @PreAuthorize("hasPermission(#id, #permissionName)")
    public ResponseEntity<APIResponse<Boolean>> hasPermission(
            @PathVariable UUID id,
            @PathVariable String permissionName) {

        return ResponseEntity.ok(
                APIResponse.success("Permission checked",
                        userService.userHasPermission(id, permissionName))
        );
    }
}
//
